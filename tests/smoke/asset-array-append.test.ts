...
let assetPath = `${TEST_PREFIX}/${assetName}`;
...
afterAll(async () => {
  if (hasSmartObjects) {
    await callBridge(bridge, "save_asset", { assetPath }).catch(() => {});
...
describe("asset - append reflected TArray elements", () => {
  it("appends native USTRUCT values atomically and emits a working rollback", async ({ skip }) => {
    ...
    assetPath = (created.result as { path?: string })?.path ?? assetPath;
    ...
    expect(invalid.error).toContain("elements[1]");
