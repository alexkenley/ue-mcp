...
A task is a named unit of work. UE-MCP ships with **<!-- count:actions -->1092+<!-- /count --> built-in tasks** across <!-- count:tools -->24<!-- /count --> categories - every action available through the MCP tools is also a flow task.
...
You rarely need to define tasks yourself - the built-in defaults cover all <!-- count:actions -->1092+<!-- /count --> actions. You define tasks when you want to **override** or **add** custom ones.
...
| Layer | File | Purpose |
|-------|------|---------|
| 1 (base) | Built-in defaults | All <!-- count:actions -->1092+<!-- /count --> tasks, no flows |
