...
- **Value forms.** `.OneOfForms({ EMCPValueForm::ArgMap, EMCPValueForm::StringList, EMCPValueForm::ArgEntryList, EMCPValueForm::String })` on an `Any` parameter or field lists the named shapes it takes: a name-to-value map, a list of strings, a `[{name, value}]` entry list, a string. Each form has one fixed, fully typed schema, so the value is advertised without the untyped member #811 removed, and a value that fits none is refused with a message naming every form.
