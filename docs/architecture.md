...
`animation` was the pilot; `pcg`, `niagara`, `material` and `widget` have followed. Handlers without a spec are declared by hand as before. Two kinds stay that way on purpose:
...
34 C++ handler groups are registered in `BridgeServer.cpp`. Together they expose <!-- count:actions -->1965+<!-- /count --> method names (some of which are aliases mapped onto a smaller number of canonical handlers):
