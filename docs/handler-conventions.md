...
- **Declare the value's real shape.** A colour is `EMCPParamType::Color`. A number that may also be a colour is `.Or(EMCPParamType::Color)`, a reference that null clears is `.Nullable()`, a flag with one legal value is `.Literal(false)`, and an array of objects declares its element with `.Items(EMCPParamType::Object).WithFields({ MCPParam::RequiredField(...), MCPParam::OptionalField(...) })`. The contract test sends a literal's own value and a union's first type.
