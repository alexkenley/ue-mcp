...
export const landscapeTool: ToolDef = categoryTool(
  ...
  "Landscape terrain: info, layers, sculpting, weight painting, materials, splines, proxies.",
  {
    ...
    list_proxies:      bp("Enumerate loaded World Partition LandscapeStreamingProxy actors with per-proxy worldBounds (origin/extent), plus loadedProxies + parentLandscapes counts. Unloaded proxies are not spawned as actors, so only loaded ones appear - use this to confirm a proxy is streamed in before trusting a layer/height readback. Params: none (#733)", "list_landscape_proxies"),
