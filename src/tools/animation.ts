...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    read_sequence:        bp("read", paged("Read anim sequence: length, rate scale, frames, root motion, curves and notifies. Each notify carries triggerTime and duration in seconds, trackIndex and trackName, and for a notify state its notifyStateClass and endTime. Resolves the same path forms asset(read) does. Batch: pass assetPaths (up to 1000) or directory (with recursive? and nameFilter?, a case-insensitive substring or * ? wildcard on the asset name) instead of assetPath to get one row per sequence in 'sequences', paged (default 50, max 200). A sequence that fails to load is its own row with success false and error; the call still succeeds and reports failedCount. fields selects the columns (assetPath is always kept); batch rows default to name, sequenceLength, rateScale, numberOfFrames, samplingFrameRate, rootMotionEnabled, notifyCount. Params: assetPath? OR assetPaths? OR directory?, recursive?, nameFilter?, fields?"), "read_anim_sequence", (p) => ({ assetPath: p.assetPath, assetPaths: p.assetPaths, directory: p.directory, recursive: p.recursive, nameFilter: p.nameFilter, fields: p.fields, cursor: p.cursor, limit: p.limit })),
    ...
    reverse_sequence:     bp("mutate", "Create a time-reversed copy of an AnimSequence, or reverse it in place with inPlace=true. Bone tracks are rewritten through the animation data controller, float and transform curves are mirrored with their tangents, notifies move to length - time (a notify state to length - time - duration) and sync markers mirror the same way. A blendspace sample with RateScale -1 is not a substitute: it freezes under the non-legacy sample length calculation. cycleOffsetFrames or cycleOffsetSeconds (rounded to whole frames) then rotates the reversed loop so it starts that far in; the last key is rewritten to repeat the new first key. The destination is destinationPath, or name (default <Source>_Reversed) in packagePath (default the source's folder). onConflict=skip (default) returns an existing destination untouched, error refuses; it never overwrites. Returns path, numFrames, numKeys, frameRate, the applied cycle offset, reversed counts {boneTracks, floatCurves, transformCurves, notifies, notifyStates, syncMarkers}, and a delete_asset rollback, or for inPlace a reverse_sequence replay that restores it. Params: sourcePath, destinationPath? | name? + packagePath?, inPlace?, cycleOffsetFrames? | cycleOffsetSeconds?, onConflict? (#1162)", "reverse_sequence", (p) => ({ sourcePath: p.sourcePath, destinationPath: p.destinationPath, name: p.name, packagePath: p.packagePath, inPlace: p.inPlace, cycleOffsetFrames: p.cycleOffsetFrames, cycleOffsetSeconds: p.cycleOffsetSeconds, onConflict: p.onConflict })),
  ...
  undefined,
  {
    ...
    facingBones: z.array(z.string()).optional().describe("analyze_animation: [boneA, boneB]. Reports the yaw of the boneA to boneB vector relative to the root bone's forward, per sample and averaged over the clip."),
    ...
    sourcePath: z.string().optional().describe("reverse_sequence: the AnimSequence to reverse (#1162)"),
    destinationPath: z.string().optional().describe("reverse_sequence: asset path for the reversed copy; alternative to name + packagePath (#1162)"),
    inPlace: z.boolean().optional().describe("reverse_sequence: reverse the source itself instead of writing a copy (default false) (#1162)"),
    cycleOffsetFrames: z.number().int().optional().describe("reverse_sequence: rotate the reversed loop by this many frames (#1162)"),
    cycleOffsetSeconds: z.number().optional().describe("reverse_sequence: rotate the reversed loop by this many seconds, rounded to whole frames (#1162)"),
    // PoseSearch (v0.7.15)
    ...
    // #153 / #154
    assetPaths: z.array(z.string()).optional().describe("Asset paths (batch) for set_sequence_properties, read_sequence and scan_animation_tracks"),
    nameFilter: z.string().optional().describe("read_sequence with directory: case-insensitive substring, or * ? wildcard, the asset name must match"),
    fields: z.array(z.string()).optional().describe("read_sequence: result keys to return, e.g. ['sequenceLength','rateScale','numberOfFrames','rootMotionEnabled','notifyCount']. assetPath is always kept; an unknown name is refused with the valid list"),
