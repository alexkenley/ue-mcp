...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    set_live_post_process_anim_blueprint: bp("Set or clear a transient post-process AnimBP override on one live SkeletalMeshComponent in the editor world or PIE. Pass the AnimBlueprintGeneratedClass object path (for example /Game/Animations/ABP_Name.ABP_Name_C), not the AnimBlueprint asset path; pass clear=true to remove the component override and fall back to the skeletal mesh asset setting. Reads back the override, effective class, and live post-process instance. This never edits or saves a mesh, Blueprint, or component template. Params: actorLabel OR actorPath, animBlueprintClassPath? OR clear=true, componentName?, world? (auto|pie|game|editor)", "set_live_post_process_anim_blueprint", (p) => ({ actorLabel: p.actorLabel, actorPath: p.actorPath, animBlueprintClassPath: p.animBlueprintClassPath, clear: p.clear, componentName: p.componentName, world: p.world })),
  },
  undefined,
  {
