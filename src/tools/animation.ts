...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    add_blend_sample:     specBp("mutate", "Append a sample to a BlendSpace (#248).", "add_blend_sample"),
    set_blend_sample:     specBp("mutate", "Move an existing BlendSpace sample or swap its animation (#272).", "set_blend_sample"),
    ...
    populate_blendspace:  specBp("mutate", "One-call axis params + samples authoring for BlendSpace 1D/2D. Axis params come from axis (with axisIndex), the per-axis blendspaceAxes list, or the flat create_blendspace keys (#459).", "populate_blendspace"),
    ...
    set_bone_keyframes:   specBp("mutate", "Set bone transform keyframes.", "set_bone_keyframes"),
    bake_keyframes_batch: specBp("mutate", "Bake per-bone keyframe arrays for many bones into an AnimSequence in one call. Auto-creates each bone track first (set_bone_keyframes silently leaves a T-pose if the track is missing), wraps the batch in one transaction, and raises if any bone fails instead of reporting hollow success (#540).", "bake_keyframes_batch"),
    ...
    set_anim_curve_keys:  specBp("mutate", "Set float-curve key VALUES on an AnimSequence (add_curve only creates an empty named curve - it cannot set keyframe values). Adds the curve if missing, then replaces its keys. Use for authoring Distance/Speed/any float curve directly (#712).", "set_anim_curve_keys"),
    ...
    create_ik_rig:        specBp("mutate", "Create IKRigDefinition asset, optionally with retargetRoot + chains[].", "create_ik_rig"),
    ...
    configure_ik_rig:     specBp("mutate", "UE 5.8 only. Author an existing IK Rig through UIKRigController with strict bone, ancestry, goal, and setting validation, native readback, one transaction, and checked save; older engines return unsupported_engine_version. autoSetup='retarget' installs the native retarget definition; 'full_body' installs the retarget definition then Full Body IK before requested desired-state upserts.", "configure_ik_rig"),
    ...
    bake_control_rig_edit: specBp("mutate", "UE 5.8 only. Bake the evaluated Control Rig session to a new AnimSequence asset; native returns unsupported_engine_version on older engines and has no raw-track fallback. The source LevelSequence remains unchanged. outputAssetPath is the output natural key; onConflict is skip|error (default error), never overwrite. Key reduction and Sequencer links are not supported yet, so reduceKeys/createLink must be false or omitted. Returns output asset metadata, frame/rate counts, status, and delete-created-asset rollback.", "bake_control_rig_edit"),
    ...
    set_bone_retargeting: specBp("mutate", "Set each bone's translation retargeting mode, which lives in the skeleton's private BoneTree and has no addressable UPROPERTY, so set_property cannot reach it. Omitting bones applies to every bone; includeChildren uses the engine's own recursive setter. Returns the prior and new mode per bone, and a per-bone restore rollback that replays through this same action.", "set_bone_retargeting"),
    ...
    create_ik_retargeter: specBp("mutate", "Create IKRetargeter asset and (default) initialize the UE 5.7 ops stack: assigns sourceRig+targetRig to all ops, runs AutoMapChains. Returns chainsMapped count (#246).", "create_ik_retargeter"),
    ...
    batch_retarget_animations: specBp("mutate", "Bake validated source AnimSequences onto the target skeleton through an IK Retargeter (RunBatchRetarget), save every output, and roll back newly created outputs if the batch is incomplete or unsavable. Overwrite is rejected. Returns mapping completeness and every unmapped target chain so partial retargets are explicit; pass requireCompleteMapping=true only when the target should have no intentional extra chains (#701).", "batch_retarget_animations"),
    ...
    set_pose_search_clips:       specBp("mutate", "Author the whole clip list of a PoseSearchDatabase in one call (the 'duplicate a stock PSD, swap its clips' pipeline step). Replaces the list by default. Each clip carries per-entry flags. Follow with build_pose_search_index (#684).", "set_pose_search_clips"),
    ...
    add_pose_search_schema_pose_channel: specBp("mutate", "Add a Pose feature channel to a schema (samples named bones for velocity/position/rotation/phase) (motion matching).", "add_pose_search_schema_pose_channel"),
    ...
    create_mirror_data_table:    specBp("mutate", "Create a MirrorDataTable for a skeleton (needed for mirrored poses in motion matching / mirror nodes). Auto-derives bone-pair rows from find/replace expressions (defaults to UE mannequin _l/_r suffix swap).", "create_mirror_data_table"),
    ...
    create_pose_search_normalization_set: specBp("mutate", "Create a PoseSearchNormalizationSet grouping databases so they normalize their cost space together (consistent blending across a locomotion set). Assign it via set_pose_search_database_settings(normalizationSetPath) (motion matching).", "create_pose_search_normalization_set"),
    ...
    measure_natural_speed: specBp("read", "Measure a locomotion clip's planted-foot speed, in cm/s, by evaluating the pose and tracking the lowest foot's horizontal travel while it is in contact. Every retarget changes natural speed by the target skeleton's leg-length ratio, so a BlendSpace built on retargeted clips has to re-measure per clip per character (#923). Returns naturalSpeed, plantedDistance, plantedTime, contactThreshold, per-foot breakdown.", "measure_natural_speed"),
  ...
  undefined,
  {
    ...
    // apply_control_rig_edits stays hand-authored: operations is a union of
    // eight object shapes, which a C++ spec cannot declare.
