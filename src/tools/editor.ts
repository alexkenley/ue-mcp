...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    stop_editor: {
      description: "Close Unreal Editor gracefully (asks the editor to quit itself via the bridge; never an OS kill). Acts only on the editor for the loaded project, resolved from the port lockfile that editor published at <project>/Saved/UE_MCP_Bridge/port.json. With no lockfile there is no port to aim at and the call refuses, naming the file it checked, rather than probing a default port that another project's editor could answer on (#819). Discards unsaved work: the shutdown save prompts (\"Save Content\", \"Save Changes\", \"Unsaved\", \"Untitled\") are armed to answer \"Don't Save\" before the quit goes out, because that prompt is a Slate modal window nothing else can answer and it otherwise parks the game thread until a human clicks it. Save first with editor(save_dirty), or use editor(request_editor_shutdown), which refuses on a dirty package. Params: none",
      handler: async (ctx: ToolContext) => {
        return stopEditor(false, ctx.project.projectDir ?? undefined);
      },
    },
    restart_editor: {
      description: "Stop then start the editor for the loaded project. Editors for other projects are left alone: the stop is aimed by this project's port lockfile, and the decision to start is made from the process holding this project's .uproject open, never from whether some editor is running (#819). Params: none",
      handler: async (ctx: ToolContext) => {
    ...
    set_dialog_policy: bp("Auto-answer dialogs whose title or message contains a pattern, including the Slate modal windows the editor raises itself (the shutdown \"Save Content\" prompt among them, whose buttons are Save Selected / Don't Save / Cancel). A response keyword resolves to whichever of the dialog's buttons carries that meaning, so response='no' presses \"Don't Save\"; pass buttonLabel to name a button literally instead. A policy set here answers a matching dialog whoever raised it, and answers one that is already on screen. Params: pattern, response? (yes/no/ok/cancel/retry/continue/yesall/noall), buttonLabel?", "set_dialog_policy"),
    clear_dialog_policy: bp("Clear dialog policies. Params: pattern?", "clear_dialog_policy"),
    get_dialog_policy: bp("Get current dialog policies, each with its response, literal buttonLabel and whether it came from a caller or is one of the plugin's built-in safety nets. Params: none", "get_dialog_policy"),
    list_dialogs: bp("List active modal dialogs, with title, message text and button labels, plus which armed policy matches and which button that policy would press (or why it presses nothing). Runs even while a dialog is blocking the editor, when every other handler times out. Params: none", "list_dialogs"),
