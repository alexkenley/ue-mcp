...
export const editorTool: ToolDef = categoryTool(
  ...
  {
    start_editor: {
      description: "Launch Unreal Editor and BLOCK until it is fully ready (not merely until the socket answers), rendering a startup progress bar in the terminal. Returns the phase timeline it waited through. Do NOT poll get_engine_state or get_status afterwards: this call already waited, and a ready editor is the only way it returns success. Idempotent: an editor already running for this project reports alreadyRunning=true with the port it published, as a success that spawned nothing, rather than refusing - so a flow step that makes sure the editor is up walks on when it already is. bridgeReady says whether that editor is answering yet. dialogPolicy answers startup prompts before they can wedge the game thread, which is how the post-crash \"Restore Packages\" modal used to stall a launch (#968). Params: timeout? (seconds, default 300), dialogPolicy? (\"pattern=response;pattern=response\", responses as set_dialog_policy takes them)",
    ...
    play_in_editor: bp("PIE control. Idempotent in both directions: a start with a session already active reports alreadyRunning, and a stop with none active reports alreadyStopped, both as successes that changed nothing, so a flow step that makes sure play is running (or stopped) does not abort on the common path where it already was. Params: pieAction (start|stop|status), waitForAssetRegistry? (start only; default true - block until the AssetRegistry initial scan completes before requesting PIE, otherwise PIE silently no-ops on cold editor starts), assetRegistryTimeoutSeconds? (default 180) (#406)", "pie_control", (p) => ({ action: p.pieAction ?? "status", waitForAssetRegistry: p.waitForAssetRegistry, assetRegistryTimeoutSeconds: p.assetRegistryTimeoutSeconds })),
    ...
    request_editor_shutdown: {
      description: "Ask the editor to close itself from inside the engine, after it has checked that closing is safe. Refuses by default when any content or map package is dirty (including an unsaved /Temp world) and reports which ones, so nothing is lost to a silent discard. Ends an active PIE/SIE session first and closes only once play has actually stopped. The response is returned before the process exits. Aimed at the same editor stop_editor aims at, through the same ownership check, so the two can never disagree about which editor belongs to the loaded project (#967), including the idempotent verdict: no editor of this project running reports alreadyStopped=true on both rather than failing on one. This IS the dirty check stop_editor runs: stop_editor calls it with requireClean=true before it sends anything, so both actions refuse on the same packages for the same reason, and neither has a way to close over unsaved work. Use editor(stop_editor) for the full stop-and-confirm flow; this action is the in-engine half of it. Params: requireClean? (default true), endPIE? (default true)",
      handler: async (ctx: ToolContext, p: Record<string, unknown>) => {
        ...
        if (!ownership.owned) {
          ...
          // disagree about one editor, so the idempotent verdict has to match
          // too: an editor that is already gone is the request satisfied on
          // both, not satisfied on one and refused on the other.
          if (ownership.alreadyStopped) {
            return { success: true, alreadyStopped: true, message: ownership.message };
          }
