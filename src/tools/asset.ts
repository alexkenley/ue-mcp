...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    render_thumbnail: bp("Render an asset PNG with Unreal's native thumbnail renderer. Supports renderable assets including static meshes, skeletal meshes, materials, and textures without level staging. Params: assetPath, outputPath, width?, height?", "render_asset_thumbnail", (p) => ({ assetPath: p.assetPath, outputPath: p.outputPath, width: p.width, height: p.height })),
    ...
    append_array_elements: bp("Append one or more JSON values to a reflected TArray without replacing existing entries. Supports dotted property paths plus native and user-defined USTRUCT elements. All elements are validated before mutation; the result includes appendedIndices and rollback data. Leaves the package dirty without saving. Params: assetPath, propertyName, elements", "append_asset_array_elements", (p) => ({ assetPath: p.assetPath ?? p.path, propertyName: p.propertyName, elements: p.elements })),
  ...
  undefined,
  {
    ...
    width: z.number().int().min(1).max(4096).optional().describe("render_thumbnail width in pixels (default 256, max 4096)"),
    height: z.number().int().min(1).max(4096).optional().describe("render_thumbnail height in pixels (default 256, max 4096)"),
