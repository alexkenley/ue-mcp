...
export const assetTool: ToolDef = categoryTool(
  ...
  {
    ...epicSchema,
    save: z.boolean().optional().describe("set_property / append_array_elements / create_subobject / import_stringtable_csv / bulk_set_properties / bulk_upsert_data_assets / import_texture_batch / set_mesh_materials_batch / fixup_redirectors / add_input_mapping / remove_input_mapping: write the changed package to disk (default true). false leaves the change in memory only and reports persisted=false with the reason; IMC aliases also persist pending edits on a no-op and report saved, packageDirty, packageName, and persistError (#931)."),
