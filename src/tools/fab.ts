...
export const fabTool: ToolDef = categoryTool(
  ...
  "Import Fab (Epic marketplace) content: check plugin/login status, trigger login/logout, sync your owned library into the Content Browser, inspect/clear the download cache, and import owned or local source files into the project.",
  {
    ...
    login:        bp("Trigger the Fab login flow (EOS account portal). Asynchronous - returns once the flow is opened, not once authenticated. Complete any prompt, then call status. Params: none", "fab_login", () => ({})),
    logout:       bp("Clear the persistent Fab authentication for this device. Params: none", "fab_logout", () => ({})),
    ...
    clear_cache:  bp("Delete the local Fab download cache to reclaim disk. Does not affect assets already imported into the project. Params: none", "fab_clear_cache", () => ({})),
